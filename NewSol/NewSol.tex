...
\title{A New Pseudo-Solution to Hydrogen}
...
In this talk, I'll start with an introduction to wave mechanics,
presenting the concept of a wave function, the Schr\"odinger Equation as a PDE,
stationary solutions, and deriving the classical solutions the Hydrogen Schr\"odinger Equation.
Then I'll exhibit a new pseudo-solution that solves the hydrogen
differential equation but doesn't meet the global square-integrability
requirement, show how I found this solution and discuss further
research directions, with the particular goal of finding a similar
solution to helium's ground state.
\end{block}
\end{frame}

\begin{frame}
\frametitle{A Link to this Slide Deck}
\begin{center}
\includegraphics[width=0.5\textwidth]{NewSol.png}
{\tt https://www.freesoft.org/bbaccala/NewSol.pdf}
\end{center}
\end{frame}
...
\begin{block}{The Euler-Lagrange (E-L) equation}
...
The first part, dependent on $r$, is the {\it radical equation}, whose solutions are, in general,
...
\[ F=e^{mi\phi} \qquad \frac{d^2 F}{d\phi^2} = - m^2 e^{a\phi} \]
...
\draw[color=red] (0,0,0) -- (P) node[midway, above] {$r$};
...
\[ m^2 e^{m\phi} - C_\phi e^{m\phi} = 0 \qquad\Longrightarrow\qquad m^2 = C_\phi \]

\[ e^{mi\phi} = e^{mi(\phi+2\pi)} \qquad\Longrightarrow\qquad e^{2\pi mi} = 1 \qquad\Longrightarrow\qquad m \in {\mathbb Z} \]
...
\includegraphics[page=1, clip, trim=0in 4.7in 0in 3.5in, width=\textwidth]{Legendre function - Wikipedia.pdf}
\includegraphics[page=3, clip, trim=0in 3.8in 0in 5.8in, width=\textwidth]{Legendre function - Wikipedia.pdf}
\vfill
Source: Wikipedia, {\it Legendre function}
\end{frame}

\begin{frame}
\frametitle{First five Legendre functions of the second kind}
\begin{center}
\includegraphics[width=0.9\textwidth]{mplwp_legendreQ04.png}
\end{center}
Source: Wikipedia, {\it Legendre function}
\end{frame}
...
suggesting an asymptotic exponential solution.

% of the form:
% \[ R(r) \sim e^{\pm \sqrt{2E}ir} \qquad R(r) = e^{\pm \sqrt{2E}ir} L(r) \]

\vfill
Source: Sommerfeld (Brose, Trans.). {\it Wave Mechanics}, 1930. Chap 1 \S 7.
...
Leibniz notation: $\frac{\delta^2 \Psi(x,y,z)}{\delta x^2}$
...
\vskip 12pt
      \begin{itemize}
        ...
        \item Ex: $x^2-1=(x+1)(x-1)$
      \end{itemize}
};
\node (algebraic geometry) [draw, node distance=120pt, below of=commutative algebra, text width=200pt] {Algebraic Geometry
\vskip 12pt
      \begin{itemize}
        \item indeterminates now take values in a field like $\mathbf{C}, \mathbf{R}, \mathbf{Q}, \overline{\mathbf{Q}}, \mathbf{F}_p$
        \item $x^2-1=0$ when $x=1$ or $x=-1$
      \end{itemize}
};
...
%%\[ \left(v_{3}, v_{2}, v_{1}, b_{1}, b_{0} - c_{0} v_{0}, 2 a_{1} + b_{0}, a_{0}, E c_{0} - c_{1} v_{0}, v_0^2 c_1 - E b_0\right) \]
\[ \left(v_{3}, v_{2}, v_{1}, b_{1}, c_{0} v_{0} - b_{0}, 2 a_{1} - b_{0}, a_{0}, E c_{0} - c_{1} v_{0}, v_0^2 c_1 - E b_0\right) \]
% old version, with typos?
...
\begin{frame}
\frametitle{Algebraic Geometry and Differential Algebra}
...
\begin{frame}
\frametitle{Helium Ground State Ansatz 16.3}
\begin{center}
\scalebox{0.9}{%
  \begin{tikzpicture}[remember picture,out=315,in=225,distance=0.4cm,node distance=40pt]
    \node (Psi) [poly, text width=100pt] {$\framebox(10,10){}\tikzmark{a}\,\Psi'' + \framebox(10,10){}\tikzmark{b}\,\Psi' + \coeff\tikzmark{c}\,\Psi$};
...
\begin{frame}[fragile]
\frametitle{I've already added an 'encoding' option to Sage}
\tiny
\begin{verbatim}
