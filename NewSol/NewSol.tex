...
Let $X \in {\mathbb R}^n$ be an algebraic variety.

\vskip 12pt

The Euclidean Distance function from $u \in {\mathbb R}^n$ to X is
\[ d_X (u) := \min_{x\in X}\ |u - x| \]
...
\begin{frame}
\frametitle{Computing the Euclidean Distance Function}
Let $q(x) = \sum_{i=0}^{n} x_i^2$ be the Euclidean quadratical form.
\begin {enumerate}
\item Pick the ring $\mathbb{Q}[u_0, ..., u_n, x_0 , ... , x_n , t]$
\item Input is the ideal $I_X$ with generators $f=(f_1,...,f_m)$
\item Let $c={\rm codim\ } X$
\item Compute $I_{X_{sing}}$ singular locus, by $c$-minors of $Jac(f)$
\item Compute the critical ideal as
\[ I u := \left( I_X + (c+1)-{\rm minors\ of} \left(\genfrac{}{}{0pt}{0}{u-x}{Jac(f)}\right)\right) : (I_{X_{sing}})^\infty \]
\item Eliminate $x_0,...,x_n$ in $I_u + (t^2 - q(x-u))$, get ${\rm EDpoly}_{X ,u} (t^2 )$
\end{enumerate}
